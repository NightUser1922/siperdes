...
<div class="container-fluid px-0">
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">Edit Bantuan Sosial</li>
        </ol>
    </nav>
    <div class="card module-hero shadow-sm border-0 mb-4">
        <div class="card-body p-4">
            <div class="d-flex gap-3 align-items-start">
                ...
                <div>
                    <span class="badge text-bg-warning mb-2">Bantuan Sosial</span>
                    <h4 class="fw-bold text-success mb-1">Edit Bantuan Sosial</h4>
                    <p class="text-muted mb-0">Perbarui data bantuan sosial sesuai catatan terbaru.</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="card-header bg-white border-0 pt-4 px-4">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <h5 class="mb-0 fw-bold text-success">Data Bantuan Sosial</h5>
                        </div>
                    </div>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ url('/bantuan-sosial/update/' . $bantuan->id_bantuan) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="nama_bantuan" class="form-label fw-semibold">Nama Bantuan <span class="required-dot">*</span></label>
                            <input type="text" class="form-control" id="nama_bantuan" name="nama_bantuan" value="{{ old('nama_bantuan', $bantuan->nama_bantuan) }}" maxlength="100" required>
                        </div>
                        <div class="mb-3">
                            <label for="instansi_pemberi" class="form-label fw-semibold">Instansi Pemberi <span class="required-dot">*</span></label>
                            <input type="text" class="form-control" id="instansi_pemberi" name="instansi_pemberi" value="{{ old('instansi_pemberi', $bantuan->instansi_pemberi) }}" maxlength="100" required>
                        </div>
                        <div class="mb-3">
                            <label for="tanggal_bantuan" class="form-label fw-semibold">Tanggal Bantuan <span class="required-dot">*</span></label>
                            <input type="date" class="form-control" id="tanggal_bantuan" name="tanggal_bantuan" value="{{ old('tanggal_bantuan', $bantuan->tanggal_bantuan) }}" required>
                        </div>
                        <div class="mb-4">
                            <label for="jumlah_penerima" class="form-label fw-semibold">Jumlah Penerima <span class="required-dot">*</span></label>
                            <input type="number" class="form-control" id="jumlah_penerima" name="jumlah_penerima" value="{{ old('jumlah_penerima', $bantuan->jumlah_penerima) }}" min="0" required>
                        </div>
                        <div class="d-flex flex-column flex-sm-row justify-content-between gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-warning px-5 rounded-pill"><i class="bi bi-save me-1"></i> Update Bantuan</button>
                        </div>
                    </form>
                </div>
            </div>
